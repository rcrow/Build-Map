...
class BuildAndSymbolize(object):
    ...
    def execute(self, parameters, messages):
        ...
        #Inputs
        templines = parameters[0].valueAsText
        ...
        ###############################################################################################################
        #Extract out shoreline from PKH db
        connectionPath = arcpy.env.scratchFolder
        arcpy.CreateDatabaseConnection_management(connectionPath, "ConnectForBackupTo_" + "igswzcwggsmoki" + "_" + "locogeo",
                                                  "POSTGRESQL", "igswzcwggsmoki", "OPERATING_SYSTEM_AUTH", "#", "#", "#",
                                                  "locogeo")
        PKHsde = connectionPath + "\\" + "ConnectForBackupTo_" + "igswzcwggsmoki" + "_" + "locogeo" + ".sde"
        pkhLines = PKHsde + "\\" + "locogeo.sde.PKHContactsAndFaults"
        #shorelines="in_memory\shorelines"
        shorelines=r"\\Igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\LOCOBigBrother.gdb\FocusAreasPartsOrOutdated\shorelines"
        arcpy.Select_analysis(pkhLines,shorelines,"type = '30.02.29'")
        lines="in_memory\lines"
        arcpy.Merge_management([shorelines,templines], lines)
