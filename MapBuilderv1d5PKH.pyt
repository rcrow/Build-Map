...
class Toolbox (object):
    def __init__(self):
        ...
        self.tools = [BuildAndCheck_Moki, BuildAndCheck_Kiva]

class BuildAndCheck_Moki(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "BuildAndCheck_Moki"
        self.description = ""
    ...
    def execute(self, parameters, messages):
        ...
        connection = r"Database Connections\Connection to igswzcwggsmoki.wr.usgs.gov.sde"

        lines = connection +r"\locogeo.sde.PKHGeologicMap\locogeo.sde.PKHContactsAndFaults"

        points = connection +r"\locogeo.sde.PKHGeologicMap\locogeo.sde.PKHMapUnitPoints"
        ...
        # Outputs
        quadTemp = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\quadTemp"
        quad = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\quad"
        selectPolys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\selectPolys"

        joinPolys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\joinedPolys"

        layerOut = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrotherFiles\LOCO_PKH_PolyCheck.lyr"
        layerName = "LOCO_PKH_PolyCheck"

        addToMXD = True

        polys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\polys"


        print("Only using active boundaries")
        checkAndDelete(quadTemp)
        arcpy.Select_analysis(inquad, quadTemp, "Active = 'yes'")

        arcpy.Union_analysis(in_features=quadTemp,
                             out_feature_class=quad,
                             join_attributes="ALL", cluster_tolerance="", gaps="GAPS")

        arcpy.DeleteIdentical_management(in_dataset=quad, fields="Shape", xy_tolerance="",
                                         z_tolerance="0")
...
class BuildAndCheck_Kiva(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "BuildAndCheck_Kiva"
        self.description = ""
        self.canRunInBackground = False
    ...
    def execute(self, parameters, messages):
        ...
        #
        # connection = connectionPath + "\\" + "ConnectForBuildMap_"+server +"_"+ db+".sde"
        connection = r"Database Connections\Connection to igswzdwgdbkiva.wr.usgs.gov.sde"
        ...
        inquad = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\LOCOBigBrother.gdb\Focus_RSC"

        layer = r"\\Igswzcwwgsrio\loco\Geology\SDE_Stuff\LayerFilesAndStyles\BuildAndCheckTemplatev2.lyr"

        # Outputs
        quadTemp = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\quadTemp_kiva"
        quad = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\quad_kiva"
        selectPolys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\selectPolys_kiva"

        joinPolys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\joinedPolys_kiva"

        layerOut = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrotherFiles\LOCO_PKH_PolyCheck_Kiva.lyr"
        layerName = "LOCO_PKH_PolyCheck"

        addToMXD = True

        polys = r"\\igswzcwwgsrio\loco\GeologicMaps_InProgress\LOCOBigBrother\PKH_BigBrother.gdb\PKH_BuildMapFiles\polys_kiva"
